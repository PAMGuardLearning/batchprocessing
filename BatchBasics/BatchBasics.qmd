...
The data are available on Zenodo at (Link to the Zenodo site). The Zenodo dataset contains three files: SoundData.zip, configuration.zip and pamguardoutput.zip. For this tutorial you only need SoundData.zip and configuration.zip. You'll be recreating the files in pamguardoutput.zip, so you don't need them, but take a look if you want to.

Unzip SoundData.zip and configuration.zip to a convenient location on your hard drive.
